...
/// </ul>
/// <a href="https://leetcode.cn/problems/max-increase-to-keep-city-skyline/">保持城市天际线</a>
pub struct Solution;

// 遍历
impl Solution {
    pub fn max_increase_keeping_skyline(g: Vec<Vec<i32>>) -> i32 {
        let n = g.len();
        let rm: Vec<_> = g.iter().map(|row| *row.iter().max().unwrap()).collect();
        let cm: Vec<_> = (0..n)
            .map(|col| g.iter().map(|row| row[col]).max().unwrap())
            .collect();
        (0..n).map(|i| (0..n).map(|j| (rm[i].min(cm[j]) - g[i][j]).max(0)).sum::<i32>()).sum()
    }
}
