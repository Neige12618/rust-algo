...
/// </ul>
/// <a href="https://leetcode.cn/problems/maximum-strictly-increasing-cells-in-a-matrix/">矩阵中严格递增的单元格数</a>
pub struct Solution;
...
impl Solution {
    pub fn max_increasing_cells(mat: Vec<Vec<i32>>) -> i32 {
        ...
        for i in 0..m {
            for j in 0..n {
                g.entry(mat[i][j]).or_insert(vec![]).push((i, j));
            }
        }
        ...
        for pos in g.values() {
            let mx: Vec<i32> = pos
                ...
                .collect();
            for (&(i, j), k) in pos.iter().zip(mx) {
                row_max[i] = row_max[i].max(k);
                col_max[j] = col_max[j].max(k);
            }
        }
...
mod test {
    ...
    fn test_1() {
        assert_eq!(
            Solution::max_increasing_cells(vec![vec![3, 1], vec![3, 4]]),
            2
        )
    }
