...
/// </ul>
/// <a href="https://leetcode.cn/problems/lexicographically-smallest-string-after-operations-with-constraint/">满足距离约束且字典序最小的字符串</a>
pub struct Solution;
...
impl Solution {
    pub fn get_smallest_string(s: String, mut k: i32) -> String {
        ...
        for c in s.iter_mut() {
            ...
            let m = (*c - b'a').min(b'a' - (*c - 26)) as i32;
            if m > k {
                *c -= k as u8;
                break;
            }
...
mod test {
    ...
    fn test_1() {
        assert_eq!(
            Solution::get_smallest_string("azza".to_string(), 3),
            "aaaa".to_string()
        )
    }
