...
/// </ul>
/// <a href="https://leetcode.cn/problems/find-a-good-subset-of-the-matrix/">找到矩阵中的好子集</a>
pub struct Solution;
...
impl Solution {
    pub fn good_subsetof_binary_matrix(grid: Vec<Vec<i32>>) -> Vec<i32> {
        ...
        for (i, row) in grid.iter().enumerate() {
            let mask = row.iter().enumerate().fold(0, |x, (i, &val)| x | val << i);
            if mask == 0 {
                return vec![i as i32];
            }
            mp.entry(mask).or_insert(i as i32);
        }
        for (x, &i) in mp.iter() {
            for (y, &j) in mp.iter() {
                if x & y == 0 {
...
mod test {
    ...
    fn test_1() {
        assert_eq!(
            Solution::good_subsetof_binary_matrix(vec![
                vec![0, 1, 1, 0],
                vec![0, 0, 0, 1],
                vec![1, 1, 1, 1]
            ]),
