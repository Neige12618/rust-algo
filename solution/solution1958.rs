...
/// </ul>
/// <a href="https://leetcode.cn/problems/check-if-move-is-legal/">检查操作是否合法</a>
pub struct Solution;
...
impl Solution {
    pub fn check_move(board: Vec<Vec<char>>, r_move: i32, c_move: i32, color: char) -> bool {
        for (dx, dy) in DIRECTION {
            ...
            if let Some(&c) = board.get(x as usize).and_then(|row| row.get(y as usize)) {
                if c == color {
                    continue;
                }
            }
            while let Some(&c) = board.get(x as usize).and_then(|row| row.get(y as usize)) {
                x += dx;
                y += dy;
                if c == color {
                    return true;
