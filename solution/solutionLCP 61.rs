...
/// <a href="https://leetcode.cn/problems/6CE719/">气温变化趋势</a>
pub struct Solution;
...
impl Solution {
    pub fn temperature_trend(temperature_a: Vec<i32>, temperature_b: Vec<i32>) -> i32 {
        ...
        temperature_a
            .iter()
            .zip(temperature_a.iter().skip(1))
            .zip(temperature_b.iter().zip(temperature_b.iter().skip(1)))
            .for_each(|((a_c, a_p), (b_c, b_p))| {
                if a_c.cmp(a_p) == b_c.cmp(b_p) {
                    cur += 1;
                    ans = ans.max(cur);
                } else {
                    cur = 0;
