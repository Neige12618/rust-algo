...
/// </ul>
/// <a href="https://leetcode.cn/problems/falling-squares/">掉落的方块</a>
pub struct Solution;
...
impl Solution {
    pub fn falling_squares(positions: Vec<Vec<i32>>) -> Vec<i32> {
        ...
        let mut helper = vec![];
        tab.insert(0, 0);
        let mut ans: Vec<_> = positions
            .into_iter()
            .map(|p| {
                let (left, size) = (p[0], p[1]);
                let right = left + size;
                let lp = *tab.range(..=left).last().unwrap().0;
                let r_height = *tab.range(..right).last().unwrap().1;
                helper.clear();
                let mut max_height = size;
                for (&key, &value) in tab.range(lp..right) {
                    max_height = max_height.max(value + size);
                    helper.push(key);
                }
                helper.iter().filter(|&k| *k >= left).for_each(|k| {
                    tab.remove(k);
                });
                tab.insert(left, max_height);
                let rp = if let Some((k, _)) = tab.range(right..).next() {
                    *k
                } else {
                    -1
                };
                if rp != right {
                    tab.insert(right, r_height);
                }
                max_height
            })
            .collect();
        (1..ans.len()).for_each(|i| ans[i] = ans[i].max(ans[i - 1]));
        ans
    }
}
