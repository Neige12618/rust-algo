...
/// </ul>
/// <a href="https://leetcode.cn/problems/number-of-possible-sets-of-closing-branches/">关闭分部的可行集合数目</a>
pub struct Solution;
...
impl Solution {
    pub fn number_of_sets(n: i32, max_distance: i32, roads: Vec<Vec<i32>>) -> i32 {
        ...
        let mut ans = 1;
        let mut f = vec![vec![vec![i32::MAX / 2; n]; n]; 1 << n];
        f[0] = g;
        for s in 1..(1 << n) as usize {
            let k = s.trailing_zeros() as usize;
            let t = s ^ (1 << k) as usize;
            let mut ok = true;
            for i in 0..n {
                for j in 0..n {
                    f[s][i][j] = f[t][i][j].min(f[t][i][k] + f[t][k][j]);
                    if ok
                        && j < i
                        && (s >> i & 1) == 1
                        && (s >> j & 1) == 1
                        && f[s][i][j] > max_distance
                    {
                        ok = false;
                    }
                }
