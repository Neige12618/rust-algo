...
/// </ul>
/// <a href="https://leetcode.cn/problems/minimum-time-to-visit-disappearing-nodes/">访问消失节点的最少时间</a>
pub struct Solution;
...
impl Solution {
    pub fn minimum_time(n: i32, edges: Vec<Vec<i32>>, disappear: Vec<i32>) -> Vec<i32> {
        ...
        let mut g = vec![vec![]; n];
        for e in edges {
            let (i, j, d) = (e[0] as usize, e[1] as usize, e[2]);
            g[i].push((j, d));
            g[j].push((i, d));
        }
        ...
        h.push(Reverse((0, 0)));
        while let Some(Reverse((cd, i))) = h.pop() {
            if cd > dis[i] {
                continue;
            }
            for &(j, d) in &g[i] {
                let nd = cd + d;
                if nd < disappear[j] && (dis[j] < 0 || nd < dis[j]) {
                    dis[j] = nd;
                    h.push(Reverse((nd, j)));
                }
            }
