...
/// </ul>
/// <a href="https://leetcode.cn/problems/apply-discount-to-prices/">价格减免</a>
pub struct Solution;
    ...
    pub fn discount_prices(sentence: String, discount: i32) -> String {
        sentence
            ...
            .map(|word| {
                let w = word.as_bytes();
                if word.starts_with('$') {
                    if let Ok(price) = String::from_utf8(w[1..].to_vec()).unwrap().parse::<u64>() {
                        return format!("${:.2}", price as f64 * (100 - discount) as f64 / 100.0);
                    }
                }
                word.to_string()
            })
