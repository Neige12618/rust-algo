...
/// </ul>
/// <a href="https://leetcode.cn/problems/count-the-number-of-incremovable-subarrays-ii/">统计移除递增子数组的数目 II</a>
pub struct Solution;
...
impl Solution {
    pub fn incremovable_subarray_count(nums: Vec<i32>) -> i64 {
        ...
        res += l as i64 + if l < n { 1 } else { 0 };
        for r in (0..n - 1).rev() {
            while l > 0 && nums[l - 1] >= nums[r + 1] {
                l -= 1;
            }
            res += l as i64 + if l <= r { 1 } else { 0 };
            if nums[r] >= nums[r + 1] {
                break;
            }
        }
